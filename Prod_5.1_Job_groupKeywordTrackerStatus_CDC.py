...
dfnew1 = pd.read_csv('s3://convosight-prod-redshift/config/5.1_Job_groupKeywordTrackerStatus.csv')
...
for i in range(no_of_table):
    try:
        ...
        if updatetime == 'not-present':
            dfirst = dfirst.filter(dfirst.createddatetime  > int(last_max)).repartition(152)
        else :
            dfirst = dfirst.filter((dfirst.updateddatetime > int(last_max)) | (dfirst.createddatetime  > int(last_max))).repartition(152)
        ...
        pre_query1 = "drop table if exists bd_staging_table_groupKeywordTrackerStatus;create table if not exists bd_staging_table_groupKeywordTrackerStatus(key_id varchar(100) not null);"
        print(pre_query1)
        
        pre_query11 = "DELETE FROM {} WHERE key_id in (SELECT key_id FROM bd_staging_table_groupKeywordTrackerStatus);".format(redshift_table_main,redshift_table_main)
        #post_query1 = "drop table bd_staging_table_groupKeywordTrackerStatus;"
        
        #print(post_query1)#"postactions":post_query1
        datasink_temp = glueContext.write_dynamic_frame.from_jdbc_conf(frame = temp_new_df, catalog_connection = "prod_dynamo_db_redshift", connection_options = {"preactions":pre_query1,"dbtable":"bd_staging_table_groupKeywordTrackerStatus", "database": "convosight_prod_dynamo_data"}, redshift_tmp_dir = args["TempDir"], transformation_ctx = "datasink_temp")# added
        ...
        for item in list_cols :
            ...
            #pre_query2 = "drop table if exists bd_staging_table_groupKeywordTrackerStatus;create table if not exists bd_staging_table_groupKeywordTrackerStatus(key_id varchar(100) not null);"
            #print(pre_query2)

            pre_query22 = "DELETE FROM {} WHERE key_id in (SELECT key_id FROM bd_staging_table_groupKeywordTrackerStatus);".format(sub_table_name,sub_table_name)
            #post_query2 = "drop table bd_staging_table_groupKeywordTrackerStatus;"
                
            #print(post_query2)#"postactions":post_query2
            #datasink_temp_subtable = glueContext.write_dynamic_frame.from_jdbc_conf(frame = temp_new_df, catalog_connection = "prod_dynamo_db_redshift", connection_options = {"dbtable":"bd_staging_table_groupKeywordTrackerStatus", "database": "convosight_prod_dynamo_data"}, redshift_tmp_dir = args["TempDir"], transformation_ctx = "datasink_temp")# added
...
dfnew1.to_csv ('s3://convosight-prod-redshift/config/5.1_Job_groupKeywordTrackerStatus.csv', index = False, header=True)
s3_target = "s3://convosight-prod-redshift/status_report/5.1_Job_groupKeywordTrackerStatus/"
