...
dfnew1 = pd.read_csv('s3://convosight-prod-redshift/config/8_Job_groupMetricsDaily.csv')
...
for i in range(no_of_table):
    try:
        ...
        dfirst = dfirst.withColumn("url", to_json("url"))
        dfirst = dfirst.withColumn("zero_comment_post_ES_data", to_json("zero_comment_post_ES_data"))
        dfirst.printSchema()
        #dfirst.show(5)
...
dfnew1.to_csv ('s3://convosight-prod-redshift/config/8_Job_groupMetricsDaily.csv', index = False, header=True)
s3_target = "s3://convosight-prod-redshift/status_report/8_Job_groupMetricsDaily/"
