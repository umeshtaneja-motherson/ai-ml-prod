...
for i in range(no_of_table):
    try:
        ...
        dfirst = dfirst.filter((dfirst.updatedatutc > last_max_timestamp) | (dfirst.createdatutc  > last_max_timestamp)).repartition(400)
        ...
        pre_query1 = "drop table if exists bd_staging_table_10;create table if not exists bd_staging_table_10(key_id varchar(100) not null);"
        print(pre_query1)
        if  updatetime == 'not-present':
            post_query1 = "begin;DELETE FROM {} WHERE createdatutc IN (SELECT createdatutc FROM (SELECT createdatutc, dense_rank() OVER (PARTITION BY key_id ORDER BY createdatutc DESC) rn FROM {})t WHERE rn > 1 and key_id in (SELECT key_id FROM bd_staging_table_10)); drop table bd_staging_table_10;end;".format(redshift_table_main,redshift_table_main)#added
        else :
            post_query1 = "begin;DELETE FROM {} WHERE updatedatutc IN (SELECT updatedatutc FROM (SELECT updatedatutc, dense_rank() OVER (PARTITION BY key_id ORDER BY updatedatutc DESC) rn FROM {})t WHERE rn > 1 and key_id in (SELECT key_id FROM bd_staging_table_10)); drop table bd_staging_table_10;end;".format(redshift_table_main,redshift_table_main)#added      
        print(post_query1)#"postactions":post_query1
        datasink_temp = glueContext.write_dynamic_frame.from_jdbc_conf(frame = temp_new_df, catalog_connection = "prod_dynamo_db_redshift", connection_options = {"preactions":pre_query1,"dbtable":"bd_staging_table_10", "database": "convosight_prod_dynamo_data"}, redshift_tmp_dir = args["TempDir"], transformation_ctx = "datasink_temp")# added
        ...
        for item in list_cols :
            ...
            pre_query2 = "drop table if exists bd_staging_table_10;create table if not exists bd_staging_table_10(key_id varchar(100) not null);"
            print(pre_query2)
            if  updatetime == 'not-present':
                post_query2 = "begin;DELETE FROM {} WHERE createdatutc IN (SELECT createdatutc FROM (SELECT createdatutc, dense_rank() OVER (PARTITION BY key_id ORDER BY createdatutc DESC) rn FROM {})t WHERE rn > 1 and key_id in (SELECT key_id FROM bd_staging_table_10));drop table bd_staging_table_10;end;".format(sub_table_name,sub_table_name)#added
            else :
                post_query2 = "begin;DELETE FROM {} WHERE updatedatutc IN (SELECT updatedatutc FROM (SELECT updatedatutc, dense_rank() OVER (PARTITION BY key_id ORDER BY updatedatutc DESC) rn FROM {})t WHERE rn > 1 and key_id in (SELECT key_id FROM bd_staging_table_10)); drop table bd_staging_table_10;end;".format(sub_table_name,sub_table_name)#added   
            print(post_query2)#"postactions":post_query2
            datasink_temp_subtable = glueContext.write_dynamic_frame.from_jdbc_conf(frame = temp_new_df, catalog_connection = "prod_dynamo_db_redshift", connection_options = {"preactions":pre_query2,"dbtable":"bd_staging_table_10", "database": "convosight_prod_dynamo_data"}, redshift_tmp_dir = args["TempDir"], transformation_ctx = "datasink_temp")# added
