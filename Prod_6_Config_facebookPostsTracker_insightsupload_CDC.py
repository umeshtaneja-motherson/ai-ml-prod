...
dfnew1 = pd.read_csv('s3://convosight-prod-redshift/config/6_Job_facebookPostsTracker_insightsupload.csv')
...
for i in range(no_of_table):
    try:
        ...
        pre_query1 = "drop table if exists bd_staging_table_6;create table if not exists bd_staging_table_6(key_id varchar(100) not null);"
        print(pre_query1)

        if  updatetime == 'not-present':
            post_query1 = "begin;DELETE FROM {} WHERE createddatetime IN (SELECT createddatetime FROM (SELECT createddatetime, dense_rank() OVER (PARTITION BY key_id ORDER BY createddatetime DESC) rn FROM {}) t WHERE rn > 1 and key_id in (SELECT key_id FROM bd_staging_table_6)); drop table bd_staging_table_6;end;".format(redshift_table_main,redshift_table_main)#added
        else :
            post_query1 = "begin;DELETE FROM {} WHERE updateddatetime IN (SELECT updateddatetime FROM (SELECT updateddatetime, dense_rank() OVER (PARTITION BY key_id ORDER BY updateddatetime DESC) rn FROM {})t WHERE rn > 1 and key_id in (SELECT key_id FROM bd_staging_table_6)); drop table bd_staging_table_6;end;".format(redshift_table_main,redshift_table_main)   #added      
        print(post_query1)#"postactions":post_query1
        datasink_temp = glueContext.write_dynamic_frame.from_jdbc_conf(frame = temp_new_df, catalog_connection = "prod_dynamo_db_redshift", connection_options = {"preactions":pre_query1,"dbtable":"bd_staging_table_6", "database": "convosight_prod_dynamo_data"}, redshift_tmp_dir = args["TempDir"], transformation_ctx = "datasink_temp")# added
        datasink1 = glueContext.write_dynamic_frame.from_jdbc_conf(frame = new_df, catalog_connection = "prod_dynamo_db_redshift", connection_options = {"dbtable":redshift_table_main, "database": "convosight_prod_dynamo_data","postactions":post_query1}, redshift_tmp_dir = args["TempDir"], transformation_ctx = "datasink1")
        
        for item in list_cols :
            if (item == 'zero_comment_post_ES_data') or (item == 'memberAgeGender'):
                ...
                sub_tab = sub_tab.withColumn('list_value', to_json('list_value'))
            ...
            pre_query2 = "drop table if exists bd_staging_table_6;create table if not exists bd_staging_table_6(key_id varchar(100) not null);"
            print(pre_query2)
            if  updatetime == 'not-present':
                post_query2 = "begin;DELETE FROM {} WHERE createddatetime IN (SELECT createddatetime FROM (SELECT createddatetime, dense_rank() OVER (PARTITION BY key_id ORDER BY createddatetime DESC) rn FROM {}) t WHERE rn > 1 and key_id in (SELECT key_id FROM bd_staging_table_6));drop table bd_staging_table_6;end;".format(sub_table_name,sub_table_name)#added
            else :
                post_query2 = "begin;DELETE FROM {} WHERE updateddatetime IN (SELECT updateddatetime FROM (SELECT updateddatetime, dense_rank() OVER (PARTITION BY key_id ORDER BY updateddatetime DESC) rn FROM {})t WHERE rn > 1 and key_id in (SELECT key_id FROM bd_staging_table_6)); drop table bd_staging_table_6;end;".format(sub_table_name,sub_table_name)#added   
            print(post_query2)#"postactions":post_query2
            datasink_temp_subtable = glueContext.write_dynamic_frame.from_jdbc_conf(frame = temp_new_df, catalog_connection = "prod_dynamo_db_redshift", connection_options = {"preactions":pre_query2,"dbtable":"bd_staging_table_6", "database": "convosight_prod_dynamo_data"}, redshift_tmp_dir = args["TempDir"], transformation_ctx = "datasink_temp")# added
            datasink_subtable = glueContext.write_dynamic_frame.from_jdbc_conf(frame = new_df_1, catalog_connection = "prod_dynamo_db_redshift", connection_options = {"dbtable": sub_table_name, "database": "convosight_prod_dynamo_data","postactions":post_query2}, redshift_tmp_dir = args["TempDir"], transformation_ctx = "datasink2")
...
dfnew1.to_csv ('s3://convosight-prod-redshift/config/6_Job_facebookPostsTracker_insightsupload.csv', index = False, header=True)
s3_target = "s3://convosight-prod-redshift/status_report/6_Config_facebookPostsTracker_insightsupload/"
